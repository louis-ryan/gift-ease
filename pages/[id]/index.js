  ...
  const getPaymentsData = async (noteId, note) => {
    try {
      const res = await fetch(
        `/api/getStripePaymentsForGift?giftId=${noteId}`,
        {
          method: 'GET',
          credentials: 'include',
          headers: {
      ...
      >
        ...
        >
          ...
          >
            {/* Background Cards */}
            {data
              .filter((item) => item.cardHTML && item.name !== 'Remaining')
              .map((item, index) => (
                <div
                  key={item.name}
                  style={{
                    ...
                    zIndex: 1,
                    ...getScatteredCardPosition(
                      index,
                      data.filter(
                        (item) => item.cardHTML && item.name !== 'Remaining'
                      ).length
                    ),
                  }}
                >
                  ...
                  >
                    ...
                    <div style={{ color: '#007bff', fontSize: '8px' }}>
                      ${item.value}
                    </div>
                  </div>
                </div>
